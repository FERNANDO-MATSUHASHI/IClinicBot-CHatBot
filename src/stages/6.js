    ...
    if (isMsgValid) {
      if (['#', '*'].includes(message)) {
        ...
        if (storage[params.from].itens.length === 0) {
          const agenda = await fetchAgenda(exame[message].description)
          ...
          msg =
            `✅ *${exame[message].description}* selecionado com sucesso! \n\n` +
            '```Data disponível```: \n\n' + dataAgendaJSON +
            '\n-----------------------------------\n#️⃣ - ```CONFIRMAR Agendamento``` \n*️⃣ - ```ENCERRAR Atendimento```'

          // Adicionado ao itens a Especialidade e a Data
          storage[params.from].itens.push(exame[message].description)
